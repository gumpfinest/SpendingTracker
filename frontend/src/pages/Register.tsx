...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (values: { name: string; username: string; password: string }) => {
    try {
      setError(null);
          ...
          {({ isSubmitting }) => (
            <Form className="space-y-4">
              <div>
                ...
                <Field type="text" name="name" className={inputClasses} placeholder="John Doe" />
                <ErrorMessage name="name" component="p" className={errorClasses} />
              </div>

              <div>
                <label htmlFor="username" className={labelClasses}>Username</label>
                <Field type="text" name="username" className={inputClasses} placeholder="johndoe" />
              ...
              <div>
                ...
                <Field type="password" name="confirmPassword" className={inputClasses} placeholder="••••••••" />
                <ErrorMessage name="confirmPassword" component="p" className={errorClasses} />
              </div>
